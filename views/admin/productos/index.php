...
<main class="panel-admin contenedor">
    <header class="panel-admin__header">
        ...
        <a href="/admin/productos/crear" class="boton boton--primario">+ Nuevo Producto</a>
    </header>

    <?php include_once __DIR__ . '/../../templates/alertas.php'; ?>

    <form action="/admin/productos" method="GET" class="panel-admin__busqueda" role="search">
        <label for="busqueda" class="visualmente-oculto">Buscar producto</label>
        <input 
            type="search" 
            id="busqueda" 
            name="busqueda" 
    ...
    <?php if (($totalPaginas ?? 1) > 1): ?>
        <nav class="paginacion" aria-label="Paginación de productos">
            <?php for ($i = 1; $i <= $totalPaginas; $i++): ?>
                <a href="/admin/productos?pagina=<?php echo $i; ?>&busqueda=<?php echo urlencode($busqueda ?? ''); ?>"
